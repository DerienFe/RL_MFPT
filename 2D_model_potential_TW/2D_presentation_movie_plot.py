...
num_simulation = 20 #number of simulations.
...
def propagate(M, cur_pos,
    ...
    return F_M, cur_pos, MM, CV_total, time_to_reach
    ...
    for i_sum in range(num_simulation):
        ...
        for prop_index in range(max_propagation):
            ...
            if closest_index == final_index:
                print(f"we have sampled the final state point, stop propagating at number {prop_index} for simulation {i_sum}")
                print(f"the time to reach at simulation {i_sum} is {time_to_reach}")
        ...
        #append time_to_reach to list and calculate the mean and std.
        time_to_reach_list.append(time_to_reach)
        time_to_reach_mean = np.mean(time_to_reach_list)
        time_to_reach_std = np.std(time_to_reach_list)

        #save the time_to_reach_list to data
        np.savetxt(f"./data/K_optimized_mfpt_raw_{state_start}_{state_end}_{time_tag}.txt", time_to_reach_list)
        np.savetxt(f"./data/K_optimized_mfpt_mean_std_{state_start}_{state_end}_{time_tag}.txt", np.array([time_to_reach_mean, time_to_reach_std]))
