...
for isim=1:Nsim
    ...
    incr_step=0;
    get_initial_guess; % initial guess from 100 random bias attempts
    v=bias;
    %get_biased_M;
    M0=expm(K*t);
    while not_reached
          v=predict_bias(M0,v,cutoff,Ngauss,amp,kT,current_pos,state_end,tot_time); % get a new v from M0 and current position.
          ...
          bias_traj{incr_step}=v; %record current gaussian params v in bias_traj.
          [prob_dist,M0] = DHAM_sym(trajectory, bias_traj, kT, N, datlength,cutoff,amp); %use DHAM, return prob_dist, updated new M0 from biased traj.
          pi_pred{incr_step}=prob_dist; %record prob_dist
    end
