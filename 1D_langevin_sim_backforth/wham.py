...
class WHAM:
    ...
    def setup(self, dist, T, global_gaussian_params):
        self.skip = int(dist.shape[1] * 0.1)
        self.KbT = 0.001987204259 * T
        self.data = dist
        self.global_gaussian_params = global_gaussian_params # comes in shape [number of prop, num_gaussian, 3]
        return
    ...
    def calculate_UB3d(self):
        ...
        num_gaussian = self.global_gaussian_params.shape[1]

        UB = np.empty(shape=(numsims, datlength, numsims), dtype=np.float32)
        U = np.empty(shape=UB.shape, dtype=np.float32) #this is the potential energy
        for i in range(numsims):
            print("Calculating UB for sim: ", i)
            for j in range(datlength):
                ...
                for k in range(numsims):
                    ...
                    for g in range(num_gaussian):
                        ...
                        u += gaussian(self.data[i, j, :], a, b, c)
                    U[i, j, k] = u
                    UB[i, j, k] = np.exp(-u / self.KbT)
        np.save("U.npy", U)
        np.save("UB3d.npy", UB)
        self.UB3d = UB
        self.U = U #this is the potential energy (sum of all bias on all CV space) on grid points of all sims
        return

    def converge(self, threshold=0.01):
        if self.UB is None:
            self.calculate_UB3d()
            #self.UB3d = np.load("UB3d.npy")
    ...
    def calc_denom(self):
        ...
        U = np.empty(shape=(numsims, datlength, numsims), dtype=np.float32)  # Potential energy
        U = self.U
        for i in range(numsims):
            print("Calculating denom for sim: ", i)
            for j in range(datlength):
                """for k in range(numsims):
                    u = 0
                    for g in range(num_gaussian):
