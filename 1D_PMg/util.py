...
def try_and_optim_M(M, working_indices, num_gaussian=10, start_state=0, end_state=0, plot = False):
    ...
    qspace = np.linspace(1.0, 8.0, 100+ 1) #hard coded for now.
    ...
    for i in range(1000): 
        ...
        b = rng.uniform(0, 8, num_gaussian) #fix this so it place gaussian at the working indices. it has to be in angstrom. because we need return these.
    ...
    res = minimize(mfpt_helper, #minimize comes from scipy.
                   ...
                   args=(M,
                         ...
                         working_indices), 
                   method='Nelder-Mead', 
                   bounds= [(0.1, 1)]*10 + [(0,10)]*10 + [(0.3, 1.5)]*10, #add bounds to the parameters
                   tol=1e-3)
