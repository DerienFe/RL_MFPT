...
class MSM:
    ...
    def _build_mfpt_matrix_M(self, check=False):
        """
        ...
        A = A.T
        Qinv = np.linalg.inv(A + I - self.M.T)
        mfpts = np.zeros((self.num_states**self.num_dimensions, self.num_states**self.num_dimensions))
        for j in range(self.num_states**self.num_dimensions):
            for i in range(self.num_states**self.num_dimensions):
    ...
    def _bias_K(self, bias):
        ...
        for i in range(self.num_states**self.num_dimensions):
            for j in range(i, self.num_states**self.num_dimensions):
                coor_i, coor_j, is_adjacent = self._is_adjacent(i,j, verbose=True)
                if is_adjacent:
                    u_ij = bias[j] - bias[i]
    ...
    def _bias_M(self, bias):
        ...
        """M_biased = np.copy(self.M)
        for i in range(self.num_states):
            for j in range(i, self.num_states):
                coor_i, coor_j, is_adjacent = self._is_adjacent(i,j)
                if is_adjacent:
                    u_ij = bias[coor_j] - bias[coor_i]
                    M_biased[i, j] *= 1 + (u_ij / max(bias))
                    M_biased[j, i] *= 1 - (u_ij / max(bias))
        #normalization on row
        M_biased = M_biased / M_biased.sum(axis=1)
        """
        self.M = M_biased.real
        print("M is biased!")
        return 0

    def _is_adjacent(self, i, j, verbose=False):
        """
