...
start_state = np.argmin(np.abs(unb_bins - 2.68))
end_state = np.argmin(np.abs(unb_bins - 7))
...
with open(f"output_files/NaCl_solvated_system_mfpt_bias_{run_number}", 'w') as file_handle:
