...
def propagate(simulation,
    ...
    file_handle = open(f"./trajectory/explore/langevin_sim_explore_{prop_index}.dcd", 'bw')
    ...
    #save the top to pdb.
    with open(f"./trajectory/explore/langevin_sim_explore_{prop_index}.pdb", 'w') as f:
        openmm.app.PDBFile.writeFile(simulation.topology, state.getPositions(), f)
    
    #we load the pdb and pass it to mdtraj_top
    mdtraj_top = mdtraj.load(f"./trajectory/explore/langevin_sim_explore_{prop_index}.pdb")
    ...
    coor_x_digitized = np.digitize(coor_xy[:,0], x) #note this is in coordinate space np.linspace(0, 2*np.pi, num_bins)
