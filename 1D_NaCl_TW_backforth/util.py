...
def try_and_optim(K=None, num_gaussian=10, start_state=0, end_state=0):
   #print("inside try_and_optim")
    ...
    for i in range(1000): 
        ...
        qspace = config.qspace #np.linspace(2.0, 12, config.num_bins+1)
        ...
        print(sum(peq))
        kemeny_constant_check(K.shape[0], mfpts_biased, peq)
    ...
    res = minimize(mfpt_helper, 
                   ...
                   args=(K,), 
                   method='Nelder-Mead', 
                   bounds= [(0, 1)]*10 + [(1, 9)]*10 + [(1.0, 5.0)]*10, #add bounds to the parameters
                   tol=1e-1)
...
def try_and_optim_M(M, working_indices, num_gaussian=10, start_state=0, end_state=0, plot = False):
    ...
    qspace = config.qspace #np.linspace(2.0, 12, config.num_bins+1) #hard coded for now.
    best_mfpt = np.inf #initialise the best mfpt np.inf
    ...
    res = minimize(mfpt_helper, #minimize comes from scipy.
                   ...
                   #method='Nelder-Mead', 
                   method='L-BFGS-B',
                   bounds= [(0.1, 1.2)]*config.num_gaussian + [(0,10)]*config.num_gaussian + [(0.3, 1.5)]*config.num_gaussian, #add bounds to the parameters
                   tol=1e-4)
