    ...
    def _init_sample_K(self, dim, plot_init_fes=False):
        ...
        if self.num_dimensions == 1:
            ...
            for i in range(num_hills):
                y += A_i[i] * 4.184 * np.exp(-(x-x0_i[i])**2/(2*sigma_x_i[i]**2))

            #add the x boundary barrier in plot
            y += float(max_barrier) * (1 / (1 + np.exp(k * (x - (-offset))))) #left
            y += float(max_barrier) * (1 / (1 + np.exp(-k * (x - (2 * np.pi + offset))))) #right
            ...
            X,Y,y = np.meshgrid(x,y,z)
            self.qspace = [X,Y,y]

            W = np.sin(X*4) + 2*np.cos(Y) + 3*np.sin(y*3)
            if plot_init_fes:
                ...
                if True:
                    ...
                    scatter = ax.scatter(X, Y, y, c=W, cmap = "coolwarm")
